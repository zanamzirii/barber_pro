...
class _BarberInviteRegisterScreenState
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final inviteCollection = inviteDoc.reference.parent.id;

      final status = (inviteData['status'] as String?)?.trim().toLowerCase();
      final used = inviteData['used'] == true;
      final validPending = inviteCollection == 'invites'
          ? status == 'pending'
          : status == 'invited';
      if (!validPending || used) {
      ...
      final branchId =
          ((inviteData['branchId'] as String?)?.trim().isNotEmpty ?? false)
          ? (inviteData['branchId'] as String).trim()
          : ((inviteData['shopId'] as String?) ?? '');
      final shopId = branchId;
      final ownerId = (inviteData['ownerId'] as String?) ?? '';

      if (shopId.isEmpty) {
        throw Exception('Invite is missing branch information');
      }
